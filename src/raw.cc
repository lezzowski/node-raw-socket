...
Napi::Value SocketWrap::BindSocket(const Napi::CallbackInfo& info) {
	...
	if (info.Length () == 0) {
		Napi::Error::New(env, "One argument required").ThrowAsJavaScriptException();
		return;
	}

	if (! info[0]->IsString ()) {
		Napi::Error::New(env, "Address argument must be a string").ThrowAsJavaScriptException();
		return;
	}

	sockaddr_in localaddr = {0};
	localaddr.sin_family = AF_UNSPEC;
	localaddr.sin_addr.s_addr = inet_addr(info[0].As<Napi::String>().Utf8Value());
